...
# Things to do still:
# - This is only run on groups of people, so maybe read in file names or something.
# - This needs to have the same blocks for different people
# - This needs to read in a template first,
#     trim the template, to reduce size.

# Create a separate function to:
# Read in scan and reduce its dimension
# Options:

# Do the same for each scan
#     then calculate sequence across all people


# Have this function read in which blocks to run.
# Blocks should be labelled, x1, x2, y1, y2, z1, z2

# Create a separate function to decide which blocks to run
# Read in image, with block size and spacing
# Output x1...z2 coordinates
